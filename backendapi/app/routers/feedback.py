...
@limiter.limit("60/minute")
def feedback_stats(request: Request, db: Session = Depends(get_db),current_user: User = Depends(get_current_user),):
    return feedback_crud.get_feedback_stats(db)


@router.put("/{feedback_id}", response_model=FeedbackResponse)
@limiter.limit("20/minute")
def update_feedback(request: Request, feedback_id: str, payload: FeedbackCreate, db: Session = Depends(get_db),current_user: User = Depends(get_current_user),):
...
def delete_feedback(request: Request, feedback_id: str, db: Session = Depends(get_db),current_user: User = Depends(get_current_user),):
